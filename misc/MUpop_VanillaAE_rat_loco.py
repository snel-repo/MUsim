...
session_date = '220603'
rat_name = 'dogerat'
treadmill_speeds = 3*['20']
treadmill_inclines = ['00','05','10']
general_session_info = f"{session_date}_{rat_name}" 

# get motor unit data
mu = MUsim()
for (speed,incline) in zip(treadmill_speeds,treadmill_inclines):
    session_parameters = f"{session_date}_{rat_name}_speed{speed}_incline{incline}"
    file_path = f"../rat-loco/{session_parameters}_time.npy"
    mu.load_MUs(file_path, bin_width=1)
    mu.convolve(gaussian_bw, target="session")  # SMOOTH SPIKES FOR SESSION
...
time_len_list = [iSess.shape[1] for iSess in sess_list]
time_len_diffs_list = [np.max(time_len_list)-time_len_list[ii] for ii in range(len(sess_list))]
for ii,(iSess,iDiff) in enumerate(zip(sess_list,time_len_diffs_list)):
    npad = ((0, 0), (0, int(iDiff)), (0, 0)) # pad along only the second axis (time)
    sess_list[ii] = np.pad(iSess, npad, constant_values=0)
