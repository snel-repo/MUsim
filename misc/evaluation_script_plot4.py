...
def find_best_cluster_matches(
    ...
    for iSort in range(len(scoring_metric_list)):
        ...
        if save_cluster_acc_stats_as_csv and KS_clusters_to_consider is not None:
            ...
            unit_df.to_csv(f"plot9/values_{KS_session_folder.name}_{timestamp}.csv")
            ...
            summary_df.to_csv(f"plot9/summary_{KS_session_folder.name}_{timestamp}.csv")
        ...
        metrics_df = df(
            {
                ...
                "datestring": KS_session_folder.name,
        ...
        for iMetric in range(len(metric_values)):
            ...
            for sort_type in set(sort_types):
                if sort_type == "EMUsort":
                    ...
                    if sort_type == "Kilosort4":  # make Kilosort4 slightly darker
...
if __name__ == "__main__":
    ...
    if num_dummies > 0:
        ...
        dum_clusters = np.arange(
            highest_KS_cluster + 1, highest_KS_cluster + num_dummies + 1
        )
        dum_times = int(60000) + np.zeros_like(dum_clusters).astype(
            int
