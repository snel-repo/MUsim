...
def load_int16_ephys_data(path_to_folder):
    """
    Load int16 data from a folder into a numpy array.
    """
    pass
...
## load Open Ephys data from the session
# paths to the folders containing the Open Ephys data
paths_to_OE_data_folders = [
    Path(
        "/snel/share/data/rodent-ephys/open-ephys/treadmill/sean-pipeline/godzilla/session20221117/2022-11-17_17-08-07_myo/Record Node 101/experiment1/concatenated_data/1,2,4,5,6/continuous/Acquisition_Board-100.Rhythm Data"
    ),
    Path(
        "/snel/share/data/rodent-ephys/open-ephys/treadmill/sean-pipeline/kitkat/session20230420/2023-04-20_14-12-09_myo/Record Node 101/experiment1/concatenated_data/1,2,3,4,5,7,8,9,11,12,14,15,16,17,19,20,21/continuous/Acquisition_Board-100.Rhythm Data"
    ),
    Path(
        "/snel/share/data/rodent-ephys/open-ephys/treadmill/sean-pipeline/inkblot/session20230323/2023-03-23_14-41-46_myo/Record Node 101/experiment1/concatenated_data/3,5,7,8,9,10/continuous/Acquisition_Board-100.Rhythm Data"
    ),
]
...
# take numchans directly from the structure.oebin file
ephys_data_list = [
    np.memmap(
        str(path_to_sorted_folder.joinpath("continuous.dat")),
        dtype="int16",
        mode="r",
    ).reshape(-1, 24)
    for path_to_sorted_folder in paths_to_OE_data_folders
]
set_trace()
...
else:  # this chunk uses the real data to create waveform shapes for each spike time, use spike times for each cluster to extract median waveform shape, with 61 points by -nt0 and +nt0+1
    # first, load the continuous.dat file into a numpy array
    pass
    # for
