...
# replace all usages of numpy and pandas with dask equivalents
# import numpy as np
...
class MUsim:
    ...
    def _get_dynamic_thresholds(self, threshmax, threshmin, new=False):
        ...
        if new == True:
            ...
            if len(self.MUreversal_static_units) != 0 and self.MUreversal_frac != 0:
                ...
                static_MU_idxs = np.topk(self.num_units,
        ...
        elif static_MU_idxs is not float("nan"):
            MUthresholds_orig = self.MUthreshold_original
            MUthresholds_flip = -(
                MUthresholds_orig - threshmax - threshmin
            )  # values flip within range
            MUthresholds = float("nan") * np.zeros(MUthresholds_orig.shape)  # place holder
    ...
    def load_MUs(self, npy_file_path, bin_width, load_as="session", slice=[0, 1]):
        ...
        if load_as == "session":
            ...
            sorted_MU_trial = transposed_binned_MU_data[
                :, self.argsort(-transposed_binned_MU_data.sum(axis=0))
            ]
        ...
        self.session_forces.append(np.repeat(float("nan"), len(self.force_profile)))
        self.session_yanks.append(np.repeat(float("nan"), len(self.yank_profile)))
    ...
    def sample_MUs(self, MUmode="static"):
        ...
        if MUmode == "static":
            spike_sorted_cols = self.argsort(self.units[0])
            self.units[0] = units[0][spike_sorted_cols]
        elif MUmode == "dynamic":
            spike_sorted_cols = self.argsort(self.units[0].mean(axis=0))
            self.units[0] = units[0][:, spike_sorted_cols]
        ...
        self.units[2] = np.topk(self.num_units,self.units[2]).compute()
