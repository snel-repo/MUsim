...
session_name = "godzilla_20221116_10MU_8CH_test"
sorter = "ks4"
...
cuda_device_number = "2"  # str(int(shape_jitter_amount // 2 + 1))  # "1".keys()
...
random_seed_entropy = 295921216980200951702345820409845315428  # rat
...
if "monkey" in session_name:
    ...
    clusters_to_take_from = [
        [
            ...
            1,
            0,
        ],  # new godzilla 8CH, 10MU (orig CHs) ## PAPER
...
kernel_interpolation_factor = 1.5  # 2 latest  # 1.25  # 2 if "monkey" or "human" in session_name else 1 # newest dataset for godzilla used 1.5, then 1.25 latest
threshmin_percentile = 35  # 40 for rat, 35 for monkey, 35 latest
threshmax_factor = 2.0  # 2.0 for rat, 2.5 for monkey, 3.5 latest
print(f"kernel_interpolation_factor: {kernel_interpolation_factor}")
print(f"threshmin_percentile: {threshmin_percentile}")
print(f"threshmax_factor: {threshmax_factor}")
...
def sample_MUsim_obj(seed):
    ...
    mu.kernel_interpolation_factor = kernel_interpolation_factor
    ...
    mu.threshmin = np.percentile(interp_final_force_array, threshmin_percentile)
    # fixed maximum force threshold for the generated units' response curves. Tune this for upper
    # bound of force thresholds sampled in the distribution of MUs during MU_sample()
    mu.threshmax = threshmax_factor * np.max(
        interp_final_force_array
...
if save_continuous_dat:
    ...
    continuous_dat.tofile(
        f"continuous_dat_files/continuous_{datetime.now().strftime('%Y%m%d-%H%M%S')}_{session_name}_SNR-{adjust_SNR}-{SNR_mode}_jitter-{shape_jitter_amount}std_method-{method}_{len(kinematic_csv_file_paths)}-files_{kernel_interpolation_factor}_kern-int_{threshmax_factor}_th-max_{threshmin_percentile}_th-min.dat"
    )
    ...
    sample_numbers_npy = np.save(
        f"sample_numbers_npy_files/sample_numbers_{datetime.now().strftime('%Y%m%d-%H%M%S')}_{session_name}_SNR-{adjust_SNR}-{SNR_mode}_jitter-{shape_jitter_amount}std_method-{method}_{len(kinematic_csv_file_paths)}-files_{kernel_interpolation_factor}_kern-int_{threshmax_factor}_th-max_{threshmin_percentile}_th-min.npy",
