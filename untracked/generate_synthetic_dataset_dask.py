...
def butter_lowpass_filter(data, lowcut, fs, order=2):
    ...
    return np.from_array(y, chunks=500)
...
shape_jitter_enable = True  # controls whether to jitter the waveform shapes
random_seed_entropy = 75092699954400878964964014863999053929  # None for random behavior, or a previous entropy int value to reproduce
...
chosen_bodypart_arrays = [
    kinematic_dataframe[chosen_bodypart_to_load].to_dask_array(lengths=True)
...
reference_bodypart_arrays = [
    kinematic_dataframe[reference_bodypart_to_load].to_dask_array(lengths=True)
    for kinematic_dataframe in kinematic_dataframes
]
...
interp_final_force_array = np.from_array(signal.resample(
...
N_processes = len(anipose_sessions_to_load)
