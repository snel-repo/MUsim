...
for iDateStr in dates:
    ...
    emu_median_list.append(
        np.median(np.array(emu_config_dict[iDateStr]["Results"]["emusort_scores"]))
    )
...
emu_median_arr = np.array(emu_median_list)
...
EMU_median_scores = emu_median_arr[EMU_idxs]
KS4_median_scores = emu_median_arr[KS4_idxs]
...
print(f"Overall EMUsort correlation is: {R_emu}")
R_ks4 = np.corrcoef(Kilosort4_scores, all_vals[KS4_idxs])[0, 1].round(4)
print(f"Overall Kilosort4 correlation is: {R_ks4}")
print("\n")
############################################ Overall
# overall scores
...
print(emu_median_arr)
emu_med_R = np.corrcoef(emu_median_arr, all_vals)[0, 1].round(4)
print(f"Correlation with EMUsort median score is: {emu_med_R}")
print("\n")
############################################ EMUsort
...
print(emu_median_arr[EMU_idxs])
emu_med_R_emu = np.corrcoef(emu_median_arr[EMU_idxs], all_vals[EMU_idxs])[0, 1].round(4)
print(f"Correlation with EMUsort median score is: {emu_med_R_emu}")
print("\n")
############################################ Kilosort4
...
print(emu_median_arr[KS4_idxs])
emu_med_R_ks4 = np.corrcoef(emu_median_arr[KS4_idxs], all_vals[KS4_idxs])[0, 1].round(4)
print(f"Correlation with EMUsort median score is: {emu_med_R_ks4}")
...
fig.update_xaxes(range=[x - 0 for x in [0, 0.7]])
fig.update_yaxes(range=[x - 0 for x in [0.4, 1]])
fig.update_yaxes(
    scaleanchor="x",
    scaleratio=1,
)
