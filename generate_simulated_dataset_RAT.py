...
session_name = "godzilla_20221116_10MU_8CH"  # "godzilla_20221116_10MU_8CH"  # "godzilla_20221116_10MU_14CH"  # "human_20231003_10MU"  # "human_20231003_13MU"  # "godzilla_20221117_10MU"  # "monkey_20221202_6MU"  # "godzilla_20221117_10MU"  # "monkey_20221202_6MU"
sorter = "ks4"
...
kinematics_fs = 125
ephys_fs = 30000
nt0 = 61  # 301  # 10.033 ms  # 61  # 2.033 ms
SVD_dim = 9  # number of SVD components than were used in KiloSort
num_chans_in_recording = 8  # 14  # 9  # number of channels in the recording
num_chans_in_output = 8  # 14  # 8 # desired real number of channels in the output data
...
cuda_device_number = "0"  # str(int(shape_jitter_amount // 2 + 1))  # "1".keys()

print(shape_jitter_amount)
...
if "monkey" not in session_name and "human" not in session_name:
    ...
    anipose_folder = Path(__file__).parent.joinpath("anipose")
    ...
    # ]
    files = [
        anipose_folder / "20221117-9_godzilla_speed15_incline15.csv",
        anipose_folder / "20221117-5_godzilla_speed05_incline15.csv",
        anipose_folder / "20221117-2_godzilla_speed10_incline00.csv",
        anipose_folder / "20221117-3_godzilla_speed15_incline00.csv",
        anipose_folder / "20221117-4_godzilla_speed10_incline00.csv",
        anipose_folder / "20221117-8_godzilla_speed15_incline00.csv",
        anipose_folder / "20221117-10_godzilla_speed15_incline00.csv",
        anipose_folder / "20221117-6_godzilla_speed05_incline00.csv",
        anipose_folder / "20221116-0_godzilla_speed10_incline00.csv",
        anipose_folder / "20221116-1_godzilla_speed15_incline00.csv",
        anipose_folder / "20221116-3_godzilla_speed05_incline00.csv",
        anipose_folder / "20221116-5_godzilla_speed10_incline00.csv",
        anipose_folder / "20221116-7_godzilla_speed05_incline00.csv",
        anipose_folder / "20221116-8_godzilla_speed10_incline00.csv",
        anipose_folder / "20221116-9_godzilla_speed10_incline00.csv",
    ]
    ...
    if write_kinematics_to_npy:
        kinematics_arrays_path = Path("output_files") / "kinematics_npy_files"
        kinematics_arrays_path.exists() or kinematics_arrays_path.mkdir(parents=True)
        np.save(
            kinematics_arrays_path
            / f"kinematics_array_{datetime.now().strftime('%Y%m%d-%H%M%S')}_shuffled_{kinematics_shuffle_N}_times_{shape_jitter_amount}_shape_noise_{adjust_SNR}_SNR.npy",
    ...
    if write_kinematics_to_mat:
        ...
        savemat(
            kinematics_arrays_path
            / f"kinematics_array_{datetime.now().strftime('%Y%m%d-%H%M%S')}_shuffled_{kinematics_shuffle_N}_times_{shape_jitter_amount}_shape_noise_{adjust_SNR}_SNR.mat",
            {"kinematics_array": interp_final_force_array},
        )
        # exit()
...
while (
    ...
    o_frac, o_fracs = compute_overlap_fraction(mu.deepcopy(), 1)
    print(f"Simulated overlap fractions are: {o_fracs}")
    print(f"Total simulated overlap fraction: {o_frac}")
    o_frac_real, o_fracs_real = compute_overlap_fraction(mu_real.deepcopy(), 1)
...
if save_simulated_spikes:
    spikes_path = Path("output_files") / "spikes_npy_files"
    spikes_path.exists() or spikes_path.mkdir(parents=True)
    mu.save_spikes(
        # f"synthetic_spikes_from_{session_name}_using_{chosen_bodypart_to_load}.npy"
        spikes_path
        / f"spikes_{datetime.now().strftime('%Y%m%d-%H%M%S')}_{session_name}_SNR-{adjust_SNR}-{SNR_mode}_jitter-{shape_jitter_amount}std_method-{method}_{len(kinematic_csv_file_paths)}-files",
        save_as="indexes",
    )
...
if save_continuous_dat:
    continuous_path = Path("output_files") / "continuous_dat_files"
    sample_numbers_path = Path("output_files") / "sample_numbers_npy_files"

    # make it if it doesn't exist
    continuous_path.exists() or continuous_path.mkdir(parents=True)
    sample_numbers_path.exists() or sample_numbers_path.mkdir(parents=True)
    ...
    sample_numbers_npy = np.save(
        sample_numbers_path
        / f"sample_numbers_{datetime.now().strftime('%Y%m%d-%H%M%S')}_{session_name}_SNR-{adjust_SNR}-{SNR_mode}_jitter-{shape_jitter_amount}std_method-{method}_{len(kinematic_csv_file_paths)}-files.npy",
