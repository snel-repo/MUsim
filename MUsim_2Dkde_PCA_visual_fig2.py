...
max_force1 = 4; max_force2 = 10 # choose max force to analyze, default is 5
yank_flip_thresh = 15
...
mu.MUthresholds_dist="normal"          # SET DISTRIBUTION OF LARGE/SMALL SIMULATED UNITS
...
force_profile1 = np.roll(force_profile1,10) # shift by 10ms
force_profile1[:10] = 0 # set first 10ms to zero force
...
force_profile2 = np.roll(force_profile2,10) # shift by 10ms
force_profile2[:10] = 0 # set first 10ms to zero force
...
plt.bar(range(num_comp_test),fit.explained_variance_ratio_*100,color='black')
...
# num_comp_test = 5
# # run for top <num_comp_test> components to see VAFs for each NMF component
...
# %% COMPUTE TRIAL AVERAGES AND DATA RANGE
# reshape into trials
...
# fig.add_trace(go.Scatter(
#     x = [min(proj12_session2_ave_x)-.1*x_range,max(proj12_session2_ave_x)+.2*x_range],
#     y = [0,0],
#     mode="lines",
#     showlegend=False,
#     line=dict(
...
# fig.add_trace(go.Scatter(
#     x = [0,0],
#     y = [min(proj12_session2_ave_y)-.8*y_range,max(proj12_session2_ave_y)+.8*y_range],
#     mode="lines",
#     showlegend=False,
#     line=dict(
#         width=1,
...
### plot results
fig10 = px.imshow(d_session1_norm.T,labels=dict(color='Probability'),zmax=0.001,title="2D PDF, yank="+str(np.round(max_force1*mu.sample_rate/mu.num_bins_per_trial,decimals=1)),width=500,height=500,origin='lower')
fig20 = px.imshow(d_session2_norm.T,zmax=0.001,title="2D PDF, yank="+str(np.round(max_force2*mu.sample_rate/mu.num_bins_per_trial,decimals=1)),width=500,height=500,origin='lower')
fig10.show(); fig20.show()

# get 95% confindence intervals
...
fig_O_1in2 = px.imshow((CI_20*d_session1_norm).T,zmax=0.001,title="yank="+str(np.round(max_force1*mu.sample_rate/mu.num_bins_per_trial,decimals=1))+" density inside 95%CI of yank="+str(np.round(max_force2*mu.sample_rate/mu.num_bins_per_trial,decimals=1))+": "+str(np.round(O_1in2,decimals=4)),width=500,height=500,origin='lower')
fig_O_2in1 = px.imshow((CI_10*d_session2_norm).T,zmax=0.001,title="yank="+str(np.round(max_force2*mu.sample_rate/mu.num_bins_per_trial,decimals=1))+" density inside 95%CI of yank="+str(np.round(max_force1*mu.sample_rate/mu.num_bins_per_trial,decimals=1))+": "+str(np.round(O_2in1,decimals=4)),width=500,height=500,origin='lower')
